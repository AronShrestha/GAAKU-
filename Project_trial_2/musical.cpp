...
musical::musical(QWidget *parent): Buyer_menu_page (parent)
{
        table5="musical";

        setFixedSize(1186,790);
        setWindowTitle("Musical Instruments");
       ...
       // scroll_back->setStyleSheet("background-color:white;");
        ...
        mydb.connOpen();
        QSqlQuery qry;
        qry.exec("select title from musical");
        ...
        for(k=0;k<count;k++){
            ...
            qry1.prepare("select * from musical where eID='"+eID+"'");
            if(qry1.exec()){
...
void musical::on_button_clicked(int x)
{

    buyer_view_window vw(x,table5);
    vw.setModal(true);
    vw.exec();
}

void musical::backbuttton_clicked()
{
    //hide();
