...
drawing::drawing(QWidget *parent) : Buyer_menu_page (parent)

{
    table2="drawinglabcoats";

    setFixedSize(1186,790);
    setWindowTitle("Drawing instruments and Labcoats");
   ...
   // scroll_back->setStyleSheet("background-color:white;");
    ...
    mydb.connOpen();
    QSqlQuery qry;
    qry.exec("select title from drawinglabcoats");
    ...
    for(k=0;k<count;k++){
        ...
        qry1.prepare("select * from drawinglabcoats where eID='"+eID+"'");
        if(qry1.exec()){
...
void drawing::on_button_clicked(int x)
{

    buyer_view_window vw(x,table2);
    vw.setModal(true);
    vw.exec();
}

void drawing::backbuttton_clicked()
{
    //hide();
