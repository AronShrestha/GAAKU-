...
electrical::electrical(QWidget *parent) : Buyer_menu_page (parent)
{
    table3="electrical";

    setFixedSize(1186,790);
    setWindowTitle("Electrical Appliances");
   ...
   // scroll_back->setStyleSheet("background-color:white;");
    ...
    mydb.connOpen();
    QSqlQuery qry;
    qry.exec("select title from electrical");
    ...
    for(k=0;k<count;k++){
        ...
        qry1.prepare("select * from electrical where eID='"+eID+"'");
        if(qry1.exec()){
...
void electrical::on_button_clicked(int x)
{

    buyer_view_window vw(x,table3);
    vw.setModal(true);
    vw.exec();
}

void electrical::backbuttton_clicked()
{
   // hide();
