...
others::others(QWidget *parent) : Buyer_menu_page (parent)
{
    table6="others";

    setFixedSize(1186,790);
    setWindowTitle("Other Items");
   ...
   // scroll_back->setStyleSheet("background-color:white;");
    ...
    mydb.connOpen();
    QSqlQuery qry;
    qry.exec("select title from others");
    ...
    for(k=0;k<count;k++){
        ...
        qry1.prepare("select * from others where eID='"+eID+"'");
        if(qry1.exec()){
             while(qry1.next()){
...
void others::on_button_clicked(int x)
{

    buyer_view_window vw(x,table6);
    vw.setModal(true);
    vw.exec();
}

void others::backbuttton_clicked()
{
    //hide();
